...
const runtimeGeneratedData = ['.build/indexnow-priority.txt','.build/indexnow-batch.txt'];
...
const report = {generated_at:new Date().toISOString(), workflow_count:files.length, workflows:files, traces, required_data:requiredData.map(p=>({path:p, exists:exists(p)})), runtime_generated_data:runtimeGeneratedData.map(p=>({path:p, exists:exists(p), packaged:false})), errors};
fs.writeFileSync(path.join(outDir,'workflow_data_trace_report.json'), JSON.stringify(report,null,2));
const md = ['# GitHub Workflow Data Trace', '', `Generated: ${report.generated_at}`, '', `Workflows: ${files.length}`, '', '## Required persistent data files', ...report.required_data.map(x=>`- ${x.exists?'PASS':'FAIL'} ${x.path}`), '', '## Runtime-generated data (excluded from baseline ZIP by contract)', ...report.runtime_generated_data.map(x=>`- ${x.exists?'PRESENT':'GENERATED-ON-RUN'} ${x.path}`), '', '## Step trace', ...traces.map(t=>`- ${t.workflow} :: ${t.job} #${t.step} :: ${t.name}${t.npm_script?` :: npm:${t.npm_script}`:''}${t.direct_paths.length?` :: paths:${t.direct_paths.join(',')}`:''}`), '', '## Errors', ...(errors.length?errors.map(e=>`- ${e}`):['- none'])].join('\n');
